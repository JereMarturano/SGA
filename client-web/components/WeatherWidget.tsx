...
import { Cloud, Sun, CloudRain, CloudSnow, CloudLightning, Droplets, Wind, Car } from 'lucide-react';
...
export default function WeatherWidget() {
    ...
    useEffect(() => {
        ...
        // Fetch weather data
        fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m&daily=precipitation_probability_max&timezone=America%2FSao_Paulo`)
            .then(res => res.json())
            .then(data => {
    ...
    if (loading) return <div className="animate-pulse h-24 w-full bg-slate-200 dark:bg-slate-700 rounded-xl"></div>;
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <div className="text-3xl font-black text-slate-800 dark:text-white tracking-tighter flex items-start">
                        {temp}°
                        <span className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-1 ml-1">C</span>
                    </div>
                    <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Molinari, Cosquín</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-xl">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <p className="text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase">Humedad</p>
                    </div>
                ...
                <div className="bg-indigo-50 dark:bg-indigo-900/20 p-3 rounded-xl">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <p className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase">Lluvia</p>
                    </div>
                ...
                <div className="bg-emerald-50 dark:bg-emerald-900/20 p-3 rounded-xl">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <p className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase">Viento</p>
                    </div>
                    <p className="text-lg font-bold text-slate-700 dark:text-slate-200">{windSpeed} <span className="text-xs">km/h</span></p>
                </div>
                <div className="bg-orange-50 dark:bg-orange-900/20 p-3 rounded-xl">
                    <div className="flex items-center gap-2 mb-1">
                        <Car size={14} className="text-orange-500" />
                        <p className="text-[10px] font-bold text-orange-600 dark:text-orange-400 uppercase">Tráfico</p>
                    </div>
