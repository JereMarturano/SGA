...
function ClientesContent() {
  ...
  const fetchClients = async () => {
    try {
      ...
      const data = response.data.map((c: any) => ({
        ...
        lastPurchase: c.ultimaCompra
          ? c.ultimaCompra.split('T')[0]
          : new Date().toISOString().split('T')[0],
        paymentMethod:
          c.metodoPagoPreferido !== null
            ? ['Efectivo', 'MercadoPago', 'Desconocido', 'Cuenta Corriente'][c.metodoPagoPreferido]
            : 'Efectivo',
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const clientData = {
      ...
      ultimaCompra: formData.get('lastPurchase')
        ? new Date(formData.get('lastPurchase') as string).toISOString()
        : null,
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                >
                  <p className="text-xs text-red-600 dark:text-red-400 mb-1 flex items-center gap-1">
                    ...
                    <span className="opacity-0 group-hover:opacity-100 transition-opacity text-[10px] ml-auto bg-white/50 px-1 rounded">
                      Gestionar
                    </span>
                  </p>
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              <select
                name="status"
                defaultValue={currentClient?.status || 'Activo'}
                className="w-full p-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
              >
          ...
          <div className="border-t border-slate-200 dark:border-slate-700 pt-4 mt-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input
                  type="number"
                  name="debt"
                  defaultValue={currentClient?.debt}
                  className="w-full p-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                  readOnly
                />
              ...
              <div className="space-y-1">
                ...
                <input
                  type="number"
                  name="totalSales"
                  defaultValue={currentClient?.totalSales}
                  className="w-full p-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                  readOnly
                />
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div className="space-y-1">
                ...
                <select
                  name="paymentMethod"
                  defaultValue={currentClient?.paymentMethod || 'Efectivo'}
                  className="w-full p-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                >
        ...
        <div className="space-y-4">
          ...
          <div className="text-center p-4 bg-slate-50 dark:bg-slate-800 rounded-xl mb-4">
            ...
            <p className="text-3xl font-black text-slate-800 dark:text-white">
              ${currentClient?.debt.toLocaleString()}
            </p>
          </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      name="amount"
                      required
                      className="w-full pl-9 p-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                      placeholder="0.00"
                    />
                  </div>
                </div>
              ...
              <div className="space-y-4">
                {saleHistory.length === 0 ? (
                  ...
                  saleHistory.map((sale) => (
                    <div
                      key={sale.ventaId}
                      className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700"
                    >
              ...
              <div className="space-y-4">
                {paymentHistory.length === 0 ? (
                  ...
                  paymentHistory.map((payment) => (
                    <div
                      key={payment.pagoId}
                      className="bg-green-50/50 dark:bg-green-900/10 p-4 rounded-xl border border-green-100 dark:border-green-900/30"
                    >
