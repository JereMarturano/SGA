              ...
              <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 border-l-4 border-l-blue-500">
                ...
                <div className="flex items-end justify-between">
                  ...
                  >
                    ...
                    {reporte.margenGananciaPorcentaje.toFixed(1)}%
                  </span>
                </div>
              </div>
                  ...
                  {reporte.ventasPorProducto.slice(0, 5).map((prod, idx) => {
                    const percentage =
                      reporte.totalVentas > 0
                        ? (prod.totalGenerado / reporte.totalVentas) * 100
                        : 0;
                    return (
                      <div key={prod.productoId} className="space-y-2">
                        <div className="flex justify-between text-sm font-medium">
                          <span className="text-slate-700 dark:text-slate-300 truncate w-2/3">
                            {prod.nombreProducto}
                          </span>
                        ...
                        <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-2.5 overflow-hidden">
                          <div
                            className={`${colors[idx % colors.length]} h-2.5 rounded-full`}
                  ...
                  <div className="mt-8 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-700">
                    ...
                    <p className="text-slate-600 dark:text-slate-300 text-sm">
                      ...
                      <span className="font-bold text-blue-600 dark:text-blue-400">
                        {topProduct.nombreProducto}
                      </span>{' '}
                      lidera las ventas con {topProduct.cantidadVendida} unidades.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700">
                ...
                <div className="space-y-4">
                  {(reporte.ventasPorMetodoPago || []).map((metodo, idx) => {
                    const percentage =
                      reporte.totalVentas > 0 ? (metodo.total / reporte.totalVentas) * 100 : 0;
                    return (
                      <div key={metodo.metodoPago} className="space-y-1">
                        <div className="flex justify-between text-sm">
                          <span className="text-slate-600 dark:text-slate-400 font-medium">
                            {getMetodoPagoLabel(metodo.metodoPago)}
                          </span>
                          <span className="text-slate-800 dark:text-white font-bold">
                  ...
                  <table className="w-full text-sm text-left text-slate-500 dark:text-slate-400">
                    <thead className="text-xs text-slate-700 uppercase bg-slate-50 dark:bg-slate-700 dark:text-slate-400">
                      <tr>
                        ...
                        <th scope="col" className="px-3 py-2 text-right rounded-r-lg">
                          Total
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      ...
                      {(reporte.topClientes || []).length === 0 && (
                        <tr>
                          <td colSpan={2} className="px-3 py-4 text-center text-xs">
              ...
              <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700">
                ...
                <div className="space-y-4">
                  {(reporte.ventasPorVendedor || []).map((vendedor, idx) => {
                    const percentage =
                      reporte.totalVentas > 0
                        ? (vendedor.totalVendido / reporte.totalVentas) * 100
                        : 0;
                    return (
                      <div key={vendedor.usuarioId} className="space-y-1">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="text-slate-800 dark:text-white font-bold">
                            $ {vendedor.totalVendido.toLocaleString('es-AR')}
                          </span>
                        </div>
                        <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-2">
                          <div
                            className={`${colors[(idx + 2) % colors.length]} h-2 rounded-full`}
                            style={{ width: `${percentage}%` }}
                          ></div>
                        </div>
                        <div className="text-right text-xs text-slate-400">
